package com.zhang.myproject.amap.view.activity

import android.content.ContentResolver
import android.content.ContentValues
import android.content.Intent
import android.net.Uri
import android.os.Bundle
import com.amap.api.maps.AMap
import com.amap.api.maps.AMapOptions
import com.amap.api.maps.model.LatLng
import com.amap.api.maps.model.Marker
import com.amap.api.maps.model.Polyline
import com.amap.api.maps.utils.overlay.MovingPointOverlay
import com.zhang.myproject.amap.R
import com.zhang.myproject.amap.databinding.ActivitySmoothMoveBinding
import com.zhang.myproject.amap.viewmodel.SmoothMoveViewModel
import com.zhang.myproject.base.activity.BaseVBVMActivity
import com.zhang.myproject.base.utils.singleClick
import me.jessyan.autosize.utils.AutoSizeUtils

class SmoothMoveActivity : BaseVBVMActivity<ActivitySmoothMoveBinding, SmoothMoveViewModel>(R.layout.activity_smooth_move) {


    // 地图
    private lateinit var mAMap: AMap

    // 轨迹线路
    private var mPolyline: Polyline? = null

    //平滑移动管理类
    private var smoothMarker: MovingPointOverlay? = null
    private var marker: Marker? = null

    //
    var resolver: ContentResolver? = null
    var contentValues: ContentValues? = null

    // 屏幕录像uri
    var mUri: Uri? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        mViewBinding.apply {

            initMap(savedInstanceState)
//        showLoadingDialog()
//        mBaseHandler.postDelayed({
//            closeLoadingDialog()
//        }, 2000)
            //Init HBRecorder
//        hbRecorder = HBRecorder(this, this)
//        addPolylineInPlayGround()
//        totalDuration()
        }
    }

    override fun initView(savedInstanceState: Bundle?) {
        TODO("Not yet implemented")
    }

    override fun setOnViewClick() {
        mViewBinding.apply {
            ivBack.singleClick {
                finish()
            }
        }
    }

    override fun createObserver() {
    }

    private fun initViewClick() {
    }

    /**
     * 申请权限。并且开始录屏。
     */
    private fun startMoveAndRecord() {/*   val permission: Array<String> = arrayOf(
            Manifest.permission.WRITE_EXTERNAL_STORAGE,
            Manifest.permission.RECORD_AUDIO
        )
        PermissionHelper.requestPermission(
            this@SmoothMoveActivity, permission, PermissionHelper.REQUEST_PER_CODE_CAMERA
        ) { requestCode: Int, isSuccess: Boolean ->
            if (requestCode == PermissionHelper.REQUEST_PER_CODE_CAMERA) {
                if (isSuccess) {
                    //首先检查是否授予了权限 检查是否正在进行录制 如果是，请停止它
                    if (hbRecorder!!.isBusyRecording) {
                        hbRecorder!!.stopScreenRecording()
                    } else {
                        startRecordingScreen()
                    }
                }
            }
        }*/
    }

    // 初始化地图
    private fun initMap(savedInstanceState: Bundle?) {
        mViewBinding.mapView.apply {
            onCreate(savedInstanceState)
//            mAMap = map
            val uiSettings = mAMap.uiSettings
            uiSettings.isZoomControlsEnabled = false
            uiSettings.isRotateGesturesEnabled = false
            uiSettings.isTiltGesturesEnabled = false
            uiSettings.logoPosition = AMapOptions.LOGO_POSITION_BOTTOM_CENTER
            uiSettings.setLogoBottomMargin(AutoSizeUtils.dp2px(this@SmoothMoveActivity, -20f))
            mAMap.setLoadOfflineData(true)
            mAMap.mapType = AMap.MAP_TYPE_SATELLITE // 卫星地图
            mAMap.isTrafficEnabled = false
        }
    }

    /**
     * 开始移动
     */
//    private fun startMove() {
//        // 读取轨迹点
//        val points = readLatLngs()
//        // 构建 轨迹的显示区域
//        val builder = LatLngBounds.Builder()
//        builder.include(points[0])
//        builder.include(points[points.size - 2])
//        mAMap.animateCamera(CameraUpdateFactory.newLatLngBounds(builder.build(), 50))
//
//        // 实例 MovingPointOverlay 对象
//        if (smoothMarker == null) {
//            // 设置 平滑移动的 图标
//            marker = mAMap.addMarker(
//                MarkerOptions().icon(BitmapDescriptorFactory.fromResource(R.drawable.feed_back_ic_car_24))
//                    .anchor(0.5f, 0.5f)
//            )
//            smoothMarker = MovingPointOverlay(mAMap, marker)
//        }
//
//        // 取轨迹点的第一个点 作为 平滑移动的启动
//        val drivePoint = points[0]
//        val pair = SpatialRelationUtil.calShortestDistancePoint(points, drivePoint)
//        points[pair.first] = drivePoint
//        val subList = points.subList(pair.first, points.size)
//        // 设置移动的监听事件  返回 距终点的距离  单位 米
//        smoothMarker!!.setMoveListener { distance ->
//            try {
//                runOnUiThread {
//                    if (distance.toInt() == 0 && hbRecorder!!.isBusyRecording) {
//                        hbRecorder?.stopScreenRecording()
//                    }
//                }
//            } catch (e: Throwable) {
//                e.printStackTrace()
//            }
//        }
//
//        // 设置轨迹点
//        smoothMarker!!.setPoints(subList)
//        // 设置平滑移动的总时间  单位  秒
//        smoothMarker!!.setTotalDuration(totalDuration())
//        // 开始移动
//        smoothMarker!!.startSmoothMove()
//    }

    // 平移时间
//    private fun totalDuration(): Int {
//        viewDataBinding.tvMileage.text.let {
//            return when {
//                it.toString().toDouble() < 1 -> 5
//                it.toString().toDouble() > 1 && it.toString().toDouble() < 10 -> 8
//                else -> 10
//            }
//        }
//    }

    /**
     * 添加轨迹线 测试
     */
//    private fun addPolylineInPlayGround() {
//        val list: List<LatLng> = readLatLngs()
//        val colorList: MutableList<Int> = ArrayList()
//        val bitmapDescriptors: MutableList<BitmapDescriptor> = ArrayList()
//        val colors = intArrayOf(
//            Color.argb(255, 0, 255, 0),
//            Color.argb(255, 255, 255, 0),
//            Color.argb(255, 255, 0, 0)
//        )
//
//        //用一个数组来存放纹理
//        val textureList: MutableList<BitmapDescriptor> = ArrayList()
//        textureList.add(BitmapDescriptorFactory.fromResource(R.mipmap.icon_ele_bike_location))
//        val texIndexList: MutableList<Int> = ArrayList()
//        texIndexList.add(0) //对应上面的第0个纹理
//        texIndexList.add(1)
//        texIndexList.add(2)
//        val random = Random()
//        for (i in list.indices) {
//            colorList.add(colors[random.nextInt(3)])
//            bitmapDescriptors.add(textureList[0])
//        }
//        mPolyline = mAMap.addPolyline(
//            PolylineOptions().setCustomTexture(BitmapDescriptorFactory.fromResource(R.mipmap.icon_ele_bike_location))
//                .addAll(list)
//                .useGradient(true)
//                .width(28f)
//        )
//        val builder = LatLngBounds.Builder()
//        builder.include(list[0])
//        builder.include(list[list.size - 2])
//        mAMap.animateCamera(CameraUpdateFactory.newLatLngBounds(builder.build(), 100))
//    }

    /**
     * 读取坐标点
     * @return
     */
    private fun readLatLngs(): MutableList<LatLng> {
        val points = mutableListOf<LatLng>()
        var i = 0
        while (i < coords.size) {
            points.add(LatLng(coords[i + 1], coords[i]))
            i += 2
        }
        return points
    }

    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        super.onActivityResult(requestCode, resultCode, data)
//        if (requestCode == 999 && resultCode == RESULT_OK) {
//            //根据 SDK 版本设置文件路径或 Uri
//            setOutputPath()
//            //开始屏幕录制
//            hbRecorder!!.startScreenRecording(data, resultCode)
//            //开始平移
//            mBaseHandler.postDelayed({
//                startMove()
//            }, 50)
    }

    /**
     * 坐标点数组数据
     */
    private val coords = doubleArrayOf(
        116.3499049793749, 39.97617053371078, 116.34978804908442, 39.97619854213431, 116.349674596623, 39.97623045687959, 116.34955525200917, 39.97626931100656, 116.34943728748914, 39.976285626595036, 116.34930864705592, 39.97628129172198, 116.34918981582413, 39.976260803938594, 116.34906721558868, 39.97623535890678, 116.34895185151584, 39.976214717128855, 116.34886935936889, 39.976280148755315, 116.34873954611332, 39.97628182112874, 116.34860763527448, 39.97626038855863, 116.3484658907622, 39.976306080391836, 116.34834585430347, 39.976358252119745, 116.34831166130878, 39.97645709321835, 116.34827643560175, 39.97655231226543, 116.34824186261169, 39.976658372925556, 116.34825080406188, 39.9767570732376, 116.34825631960626, 39.976869087779995, 116.34822111635201, 39.97698451764595, 116.34822901510276, 39.977079745909876, 116.34822234337618, 39.97718701787645, 116.34821627457707, 39.97730766147824, 116.34820593515043, 39.977417746816776, 116.34821013897107, 39.97753930933358, 116.34821304891533, 39.977652209132174, 116.34820923399242, 39.977764016531076, 116.3482045955917, 39.97786190186833, 116.34822159449203, 39.977958856930286, 116.3482256370537, 39.97807288885813, 116.3482098441266, 39.978170063673524, 116.34819564465377, 39.978266951404066, 116.34820541974412, 39.978380693859116, 116.34819672351216, 39.97848741209275, 116.34816588867105, 39.978593409607825, 116.34818489339459, 39.97870216883567, 116.34818473446943, 39.978797222300166, 116.34817728972234, 39.978893492422685, 116.34816491505472, 39.978997133775266, 116.34815408537773, 39.97911413849568, 116.34812908154862, 39.97920553614499, 116.34809495907906, 39.979308267469264, 116.34805113358091, 39.97939658036473, 116.3480310509613, 39.979491697188685, 116.3480082124968, 39.979588529006875, 116.34799530586834, 39.979685789111635, 116.34798818413954, 39.979801430587926, 116.3479996420353, 39.97990758587515, 116.34798697544538, 39.980000796262615, 116.3479912988137, 39.980116318796085, 116.34799204219203, 39.98021407403913, 116.34798535084123, 39.980325006125696, 116.34797702460183, 39.98042511477518, 116.34796288754136, 39.98054129336908, 116.34797509821901, 39.980656820423505, 116.34793922017285, 39.98074576792626, 116.34792586413015, 39.98085620772756, 116.3478962642899, 39.98098214824056, 116.34782449883967, 39.98108306010269, 116.34774758827285, 39.98115277119176, 116.34761476652932, 39.98115430642997, 116.34749135408349, 39.98114590845294, 116.34734772765582, 39.98114337322547, 116.34722082902628, 39.98115066909245, 116.34708205250223, 39.98114532232906, 116.346963237696, 39.98112245161927, 116.34681500222743, 39.981136637759604, 116.34669622104072, 39.981146248090866, 116.34658043260109, 39.98112495260716, 116.34643721418927, 39.9811107163792, 116.34631638374302, 39.981085081075676, 116.34614782996252, 39.98108046779486, 116.3460256053666, 39.981049089345206, 116.34588814050122, 39.98104839362087, 116.34575119741586, 39.9810544889668, 116.34562885420186, 39.981040940565734, 116.34549232235582, 39.98105271658809, 116.34537348820508, 39.981052294975264, 116.3453513775533, 39.980956549928244
    )

}
//    override fun onResume() {
//        super.onResume()
//        viewDataBinding?.mapView?.onResume()
//    }
//
//    override fun onPause() {
//        super.onPause()
//        viewDataBinding?.mapView?.onPause()
//    }
//
//    override fun onDestroy() {
//        super.onDestroy()
//        // 销毁平滑移动marker
//        if (smoothMarker != null) {
//            smoothMarker!!.setMoveListener(null)
//            smoothMarker!!.destroy()
//        }
//        viewDataBinding?.mapView?.onDestroy()
//        if (hbRecorder!!.isBusyRecording) hbRecorder!!.stopScreenRecording()
//    }
